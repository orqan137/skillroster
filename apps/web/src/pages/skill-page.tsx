...
export function SkillPage() {
  ...
  return (
    <AppShell member={data.member} teamName={data.snapshot.team.spec.displayName}>
      ...
      <section className="data-section reviews" id="reviews"><div className="panel-heading"><div><span className="eyebrow">스킬 후기</span><h2>v{data.skill.document.spec.version} 평가</h2></div></div>{data.reviews.length === 0 && <div className="empty-state">작성된 후기 없음</div>}{data.reviews.map((review) => <blockquote key={review.metadata.name}><div className="stars read-only">{Array.from({ length: 5 }, (_, index) => <Star key={index} size={16} fill={index < review.spec.score ? "currentColor" : "none"} />)}</div><p>{review.spec.comment}</p><footer>@{review.spec.reviewer}{review.spec.reviewer === owner ? " · 작성자 평가" : ""}{review.spec.project ? ` · ${review.spec.project}` : ""}</footer></blockquote>)}</section>
    </AppShell>
  );
}
