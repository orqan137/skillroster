...
export function SkillsPage() {
  ...
  return <AppShell member={member} teamName={snapshot.team.spec.displayName}>
    <DirectoryActionHeader eyebrow="Skills" title="스킬" description="팀 공유 스킬과 내 로컬 스킬, 평가 및 프로젝트 연결 확인" actionLabel="스킬 추가" onAction={() => setCreating({ mode: "create" })} />
    ...
    <section className="data-section skill-directory">
      <div className="directory-toolbar"><div className="directory-tabs" role="tablist"><button aria-selected={tab === "shared"} type="button" onClick={() => setTab("shared")}>팀 공유 스킬 <span>{snapshot.skills.length}</span></button><button aria-selected={tab === "local"} type="button" onClick={() => setTab("local")}>내 로컬 스킬 <span>{data.localSkills?.skills.length ?? 0}</span></button></div><label className="directory-search"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="이름·설명·태그 검색" /></label></div>
      {tab === "shared" ? <div className="shared-skill-directory">
      ...
      </div> : <div className="personal-skill-directory">
