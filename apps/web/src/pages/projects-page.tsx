...
export function ProjectsPage() {
  ...
  return <AppShell member={member} teamName={snapshot.team.spec.displayName}>
    <DirectoryActionHeader title="프로젝트" actionLabel="프로젝트 추가" onAction={() => setCreating(true)} />
    <section className="directory-stats" aria-label="프로젝트 통계">
      ...
      <article><Link2 size={20} /><span>연결된 스킬</span><strong>{connectedSkills.size}</strong></article>
      <article><Boxes size={20} /><span>팀 스킬</span><strong>{snapshot.skills.length}</strong></article>
    </section>
    <section className="data-section project-directory">
      <div className="panel-heading"><h2>프로젝트 목록</h2></div>
      ...
      {snapshot.projects.map((project, index) => {
        ...
        return <Link className="directory-row" to={`/projects/${project.metadata.name}`} key={project.metadata.name}>
          ...
          <div className="directory-main"><strong>{project.spec.displayName}</strong><p>{project.spec.tags.join(" · ") || "기술 태그 없음"}<span className={project.spec.repository ? "project-git-badge" : "project-git-badge missing"}>{project.spec.repository ? "Git 연결" : "Git 미연결"}</span></p></div>
          ...
          <div className="directory-count"><strong>{selected.length}</strong><span>스킬</span></div><ArrowUpRight size={18} />
        </Link>;
      })}
    </section>
