...
export function ProjectManageDialog({ project, displayName: initialDisplayName, initialRepository, initialTags, onClose, onDeleted, onUpdated }: { project: string; displayName: string; initialRepository: string; initialTags: string[]; onClose: () => void; onDeleted: () => void; onUpdated: (warning?: string) => void }) {
  ...
  async function save() {
    ...
    try {
      const result = await fetchJson<{ warning?: string }>(`/api/projects/${encodeURIComponent(project)}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ displayName, repository, tags }) });
      onUpdated(result.warning);
    } catch (caught) { setError(caught instanceof Error ? caught.message : String(caught)); }
    finally { setSaving(false); }
  }
  async function remove() {
    if (!window.confirm(`‘${initialDisplayName}’을 팀 로스터에서 삭제할까요? 프로젝트 Git의 .skillroster 파일은 유지됩니다.`)) return;
    setSaving(true); setError("");
    try {
  ...
  return <div className="modal-backdrop"><button className="modal-backdrop-dismiss" type="button" tabIndex={-1} onClick={onClose} aria-label="프로젝트 설정 닫기" /><section className="project-manage-modal" role="dialog" aria-modal="true" aria-label="프로젝트 설정">
    ...
    <h2>프로젝트 설정</h2><p>저장하면 팀 Git과 프로젝트 Git에 반영</p>
    <div className="project-manage-fields"><label><span>프로젝트 이름</span><input value={displayName} onChange={(event) => setDisplayName(event.target.value)} /></label><label><span>프로젝트 ID</span><input value={project} disabled /></label><label className="wide"><span>프로젝트 Git 주소</span><input required value={repository} onChange={(event) => setRepository(event.target.value)} placeholder="https://github.com/org/project" /><small>주소 변경 시 접근 권한을 다시 확인하고 프로젝트 구성 파일을 갱신</small></label><div className="tag-picker"><span>기술 태그</span><div className="tag-entry"><input value={tagInput} onChange={(event) => setTagInput(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); addTags(); } }} placeholder="react, spring" /><button type="button" onClick={addTags}>각각 추가</button></div><div className="selected-tags">{tags.map((tag) => <button type="button" onClick={() => setTags((current) => current.filter((item) => item !== tag))} key={tag}>#{tag}<X size={11} /></button>)}</div></div></div>
