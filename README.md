...
SkillRoster는 개인 컴퓨터에 흩어진 에이전트 스킬을 팀 Git 저장소에 모아 관리합니다. 팀원은 필요한 스킬만 팀에 공유하고, 사용해 본 결과와 의견을 남깁니다. 새 프로젝트를 시작할 때는 기술 태그와 팀 평가를 살펴본 뒤 필요한 스킬 버전을 골라 OpenCode·Codex·Claude Code 경로에 설치할 수 있습니다.

프롬프트, 소스 코드, 인증정보는 기본적으로 저장하지 않습니다. 팀에 공유한 `SKILL.md`, 사용자가 고른 첨부 파일과 참고 위치, 평가, 프로젝트 구성, 최소한의 실행 기록만 Git에 남습니다.
...
| 2 | 스킬 버전을 팀에 공유 | `SKILL.md`, 선택한 참고 자료, 작성자와 Git 이력 |
| 3 | 작성자·동료 평가와 프로젝트 사용 결과 기록 | 평가, 후기, 실행 상태, 사용 프로젝트 |
| 4 | 프로젝트에 필요한 스킬을 골라 도구별 경로에 설치 | 선택한 스킬 ID와 정확한 버전 |

커뮤니티 순위만 따르지 않고, 같은 팀이 남긴 평가와 프로젝트 사용 이력을 함께 보여주는 것이 핵심입니다. 자료는 일반 Markdown·YAML·JSON 문서로 저장되며, 누가 무엇을 바꿨는지는 Git의 commit·diff·blame·rollback으로 확인할 수 있습니다.
...
<table>
  ...
  <tr>
    <td><b>공유 스킬과 로컬 스킬</b><br>팀에 올린 스킬과 로컬에만 보관한 스킬 구분.</td>
...
- **필요한 스킬만 공유**: `SKILL.md`만 기본으로 올리고, 참고 파일은 사용자가 직접 고른 경우에만 포함.
...
| 팀 스킬 공유와 평가 | 지원 | 지원 | 지원 |
| 프로젝트 경로에 설치 | `.opencode/skills` | `.agents/skills` | `.claude/skills` |
| 자동 실행 기록 | 지원 | 예정 | 예정 |

평가·프로젝트 구성·Git 이력은 어떤 에이전트를 쓰더라도 함께 사용할 수 있습니다. 자동 실행 기록은 현재 OpenCode 플러그인에서만 지원합니다.

## 기록 범위

| Git에 기록함 | 기본적으로 기록하지 않음 |
|---|---|
| 팀에 공유한 `SKILL.md`와 정확한 버전 | 프롬프트와 에이전트 응답 |
...
```mermaid
flowchart LR
  A[팀원 로컬 스킬] -->|선택해 공유| B[SkillRoster 클라이언트]
...
| `publish` | 로컬 스킬을 팀 저장소에 새 버전으로 등록 |
| `review` | 특정 스킬 버전 평가 |
| `project init` | 프로젝트 등록과 OpenCode 실행 기록 설정 |
...
CLI 빌드와 팀 초기화·참여·스킬 공유 예시는 [시작하기](docs/GETTING_STARTED.md)에 정리되어 있습니다.

## 보안 경계

- 대시보드는 인증 기능이 없는 로컬 클라이언트이며 기본 주소는 `127.0.0.1`입니다.
- 팀 권한은 Git 원격 저장소의 접근 설정을 따릅니다. 화면의 팀 역할은 별도 권한 체계가 아닙니다.
- 프로젝트 검사 명령과 공유 스킬은 사용자의 컴퓨터에서 실행될 수 있습니다. 적용하기 전에 Git 변경 내용을 확인해야 합니다.
