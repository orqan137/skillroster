...
이 문서는 [2026-08-20 공개된 공식 심사기준](https://osscontest.kr/notice/41)에 맞춰 저장소에서 확인할 수 있는 자료를 정리함. 2026-08-26 기준으로 항목명과 배점을 다시 대조했으며, 설명보다 실제 실행 결과를 우선함.
...
| 프로젝트 혁신성 | 6 | 팀 내부 평가 + 프롬프트·소스를 저장하지 않는 실행 기록 + 기술 태그 추천 + Git 저장 방식 | `pnpm demo`에서 순위·추천·Git 이력 확인 |
...
| 작품 데모(완성도) | 10 | 원격 인증 없이 실행되는 `pnpm demo`, [실제 화면](../README.md#실제-화면), React 화면과 Git 자료 | 샘플 3명·3스킬·2프로젝트·4평가·2실행 기록 확인 |
...
구현되지 않은 기능은 발표에서 완료된 것처럼 설명하지 않음. 현재 구현 범위를 분명히 밝히는 것도 기술 신뢰도를 위한 기준으로 삼음.
