...
- `skills/`: 목록과 상세 화면에 표시하는 최신 공유본
- `releases/`: 같은 버전을 다시 설치할 수 있도록 보관하는 버전별 사본
...
- 이미 등록된 `owner/name/version` 조합은 다시 올릴 수 없다.
...
기본 공유 범위는 `SKILL.md`다. 직접 선택하지 않은 스크립트, 소스 코드, 인증정보, 업무 문서는 팀 Git에 복사하지 않는다.
